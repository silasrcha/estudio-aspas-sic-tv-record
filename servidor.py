"""
Servidor de desenvolvimento do Estúdio de Aspas.

Existe por um motivo só: o http.server padrão deixa o navegador cachear, e
módulos ES (adesivaco.js) são cacheados de forma agressiva — dá pra editar o
...
def main():
    porta = int(sys.argv[1]) if len(sys.argv) > 1 else 8796
    pasta = os.path.dirname(os.path.abspath(__file__))
    servidor = ThreadingHTTPServer(("127.0.0.1", porta), partial(SemCache, directory=pasta))
    print("Estudio Aspas SIC TV Record em http://localhost:%d  (pasta: %s)" % (porta, pasta))
    try:
